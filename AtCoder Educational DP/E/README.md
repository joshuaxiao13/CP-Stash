...
If ```dp[k] = 0```, that means there is no possible way to build a knapsack with a total value of ```k``` from the given items. We'll use a 64-bit integer to avoid overflow (maximum sum of weights is ```1e11```).
